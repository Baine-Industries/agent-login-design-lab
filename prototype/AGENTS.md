...
The prototype exposes a browser-preview companion from the top bar so both host treatments can be tested before native wrapping. The macOS presentation follows the verified native treatment: dark system glass, native-looking type hierarchy, restrained borders, and compact rows. The Windows presentation uses the same four groups with tighter tray geometry. Both are one Activity surface, not a second data model. Native registration, OS badges, live event transport, focus/launch, and click-away dismissal remain host integration work.

For the current OS-companion visual pass, treat the real local AppKit menu-bar proof as the visual source of truth. The `/companion` route is a comparison lab with three outside-the-popover treatments: Quiet signal (the native baseline), Action first, and Compact stack. Keep all three in the same Liquid Glass family; they are throwaway presentation tests over the same in-memory Activity fixture, not product settings. When updating the native proof, retire stale local binaries so only the current repo implementation remains visible in the toolbar.

Use the shared semantic colors for Activity states on the dots only: Pending Requests orange, Active Tasks blue, Needs Attention red, and completed Recent Activity green. Pending Requests and Needs Attention use a neutral clickable `Review` label; Active Tasks use neutral `Running` without an action affordance; Recent Activity is a green-dot log with no trailing label. Render access as `Vault Access Unlocked` or `Vault Access Locked`, green or gray respectively. Keep the macOS companion on AppKit's system font and system monospaced metadata so it blends with the operating system; the app's Inter, Manrope, and IBM Plex Mono choices belong to the Agent Vault editorial surface, not the menu-bar host.
