...
Use the shared semantic colors for Activity states: Pending Requests orange, Active Tasks blue, Needs Attention red, and completed Recent Activity green. Keep the macOS companion on AppKit's system font and system monospaced metadata so it blends with the operating system; the app's Inter, Manrope, and IBM Plex Mono choices belong to the Agent Vault editorial surface, not the menu-bar host.
