...
function App() {
  ...
  return (
    <div className={`app-shell ${darkMode ? "is-dark" : ""}`} style={{ "--type-scale": typeScale }}>
      ...
      {modal === "activity" && <ActivityModal pendingRequest={pendingRequest} recentActivity={recentActivity} onClose={() => setModal(null)} onSelectItem={openActivityItem} onSelectRequest={openActivityRequest} />}
...
function ActivityModal({ pendingRequest, recentActivity, onClose, onSelectItem, onSelectRequest }) {
...
function EditItemModal({ item, spaces, categoryOptions, reusableFields, onSave, onSaveReusableField, onClose }) {
  ...
  function submit(event) {
    event.preventDefault();
    const nextDraft = { ...draft, custom: draft.custom.map((field) => ({ ...field, siteLabel: (field.siteLabel || "").trim() })) };
